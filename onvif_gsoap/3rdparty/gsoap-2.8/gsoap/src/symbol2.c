...
compile(Table *table)
{
  ...
  if (!Sflag && !iflag && !jflag)
  {
    ...
    if (!Lflag)
    {
      ...
      fprintf(fmsg, "Saving %s client stubs with statements (use only for libs)\n", pathsoapClientLib);
  ...
  if (!Cflag && !iflag && !jflag)
  {
    ...
    if (!Lflag)
    {
      ...
      fprintf(fmsg, "Saving %s server request dispatcher with statements (use only for libs)\n", pathsoapServerLib);
      fprintf(flib, "\n\n/** Use this file in your project build instead of the two files %s and %s. This hides the serializer functions and avoids linking problems when linking multiple clients and servers. */\n", soapC, soapServer);
      /* fprintf(flib, "\n#ifndef WITH_NOGLOBAL\n#define WITH_NOGLOBAL\n#endif"); deprecated behavior */
      fprintf(flib, "\n#define SOAP_FMAC3 static");
      fprintf(flib, "\n#include \"%s\"", soapC);
...
generate_header(Table *t)
{
  ...
  if (typetable)
  {
    ...
    {
      fprintf(freport, "## Typedefs {#doc-typedefs}\n\n");
      fprintf(freport, "A typedef type is serializable if its underlying base type is serializable.  Typedefs may declare custom statements, meaning their underlying types are custom-serialized in XML using serialization rules that differ from the serialization rules of the underlying base type:");
...
generate_type(Tnode *typ)
{
  ...
  if (lflag && typ->type == Tint && !typ->sym)
  {
    ...
    fprintf(fhead, "\n#endif");
    return; /* do not generate int statements in libs */
  }
...
soap_serialize(Tnode *typ)
{
  ...
  switch(typ->type)
  {
    case Tclass:
      ...
      /* fall through to next case when class is volatile, since statements cannot be member functions */
    case Tstruct:
      if (is_external(typ) && !is_volatile(typ))
      {
        fprintf(fhead, "\nSOAP_FMAC1 void SOAP_FMAC2 soap_serialize_%s(struct soap*, const %s);", c_ident(typ), c_type_id(typ, "*"));
